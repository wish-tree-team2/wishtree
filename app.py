...
KST = timezone(timedelta(hours=9))  # 한국 표준시간대
    # 현재 시간을 한국 시간대로 설정
korea_time = datetime.now(tz=KST)
...
class Wish(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=korea_time)
...
def login():
    if request.method == 'GET':
        ...
        user = User.query.filter_by(
            user_id=user_id_receive, password=password_receive).first()
...
def add_cheering(wish_id):
    if request.method == 'POST':
        ...
        if wish:
            cheering = Cheering(
                wish_id=wish_id, comment_contents=comment_contents)
            db.session.add(cheering)
            db.session.commit()
    return redirect(url_for('get_cheerings',wish_id = wish_id))
...
@app.route('/wish/<int:wish_id>/cheerings')
def get_cheerings(wish_id):
    wish = Wish.query.get(wish_id)
    if wish:
        cheerings = Cheering.query.filter_by(wish_id=wish_id).all()
        count = len(cheerings)
    else:
        cheerings = []

    return render_template('wishTree.html', wish=wish, cheerings=cheerings, count=count)
